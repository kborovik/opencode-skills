...
## §I INTERFACES
- skill: `skills/<name>/SKILL.md` → 13 PUBLISHED skills (spec, build, check, condense, design, reorganize, explain, github, backprop, socratic, steno, telegraph, monitor)
...
V10: sub-skill-flags — internal sub-skills (github, telegraph, backprop, socratic, steno, monitor) not directly invocable; auto-fire or programmatic only; description opens "Internal — not for direct invocation" (frontmatter description detection handles YAML 1.2 block-scalar continuation lines; closes §B.15).
...
V14: mechanize-scan — every user-invocable SKILL.md carries byte-identical canonical MECHANIZE block; script audits byte-identity (DRIFT/MISSING) across the user-invocable set; exempt internal sub-skills w/o corresponding slash-command (`github`, `backprop`, `monitor`, `socratic`, `steno`, `telegraph`) (closes §B.1).
...
V33: clarity-standard — human-facing prose (steno-register surfaces per github-facing-register invariant): main-point-first, one-idea-per-sentence, plain-words (no idiom / word-metaphor / colloquialism / culture-shorthand / jargon-idiom — write literal meaning), define-or-avoid jargon (established `drift`/`bottleneck`/`leak` fine), expand-acronym-on-first-use (common web/protocol acronyms `API`/`URL`/`JSON`/`HTTP`/`JWT` exempt), spell-out-symbols per steno SYMBOLS set, citation at sentence tail (§V.<n>, #123 not sentence-opening), markdown-list-preference (human-facing docs avoid markdown tables, replace with lists whenever possible), operator-choice response states choice in one sentence + options one line each + one-sentence recommend (no trailing prose-question gate); full rules live in steno skill (SKIM TEST, SENTENCE SHAPE, SYMBOLS, BOUNDARIES carry most; net-new sub-rules: acronym-expansion, cite-at-tail, decision-format, markdown-list-preference); author-guidance only, not check-audited (no mechanical audit); orthogonal to register-selection row.
V34: release-sync-precondition — release skill recipe step #1: bidirectional sync — push local commits + tags to upstream, pull upstream commits + tags to local, so `git ls-remote --heads --tags` ref set == local ref set; abort if divergence unreconcilable or working tree dirty; ensures release tag points at the commit operator intends, release notes match release contents (closes §B.16).
...
T9|x |fix `scripts/check-mechanical.py` `is_user_invocable` YAML 1.2 block-scalar parsing — strip indent from `description: |` continuation lines, prefix-match `Internal` on any continuation line|I.script,V10,B15
...
B2|2026-06-17|original capture reversed — commit SKILL.md authored legitimately w/o MECHANIZE + `## Next` blocks (V9 no-dispatch exception); commit mis-classified user-invocable by `is_user_invocable`; path-scoping enforced via staged-index discipline not commit-cmd flag pattern|V9,V24
