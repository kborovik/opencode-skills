...
name: condense
description: |
  SPEC.md condenser — token-budget sweep. Invoke when user asks to condense
...
Operator-triggered six-prong sweep. Scope: SPEC.md + `SPEC.archive.md` + `.opencode/check-extras.md`. Not auto-fire — the check skill emits advisory when token estimate > 20k; operator invokes next turn. Single atomic commit (all firing prongs or none); rollback `git revert`. Writes serialize main-thread; per-prong scan reads delegable to sub-agents.
...
Prune history residue across live §V/§T/§B row bodies — SPEC.md is clean current design; history lives in commit log + archive. Pattern set single-sourced by freshness-contract invariant (shared w/ check skill audit + spec skill write-time prune):
...
**§T body-trim** — owned here because the build skill flips status cell only, so §T body not reachable by spec skill write-time prune: oversized `task` cell carrying step-by-step transcript → one-line goal; surplus → commit-msg body. Mirrors §B `cause` one-line trim.
...
3. Prong 6 fired → `git add .opencode/check-extras.md`.
...
Heading `## Next`; 1–5 atomic items (one sentence each, no `Reply` prefix); positional dispatch (`run <int>` or `invoke the <skill-name> skill [args]`). Optional `## Hint` (≤ 3 lines) precedes when item selection needs hidden state. State-mutator → post-EXECUTE prefer invoking the check skill (confirm cite-DAG + format-layer + token-budget clean).
...
1. invoke the check skill — cascade scan over condensed SPEC.md
...
Variants: CONFIRM cancel (no commit) → swap item 1 for: invoke the condense skill (re-run to apply after spec review), drop item 3. CONFIRM subset → Next-block unchanged.

## NON-GOALS

- not auto-fire — the check skill emits advisory; operator invokes the condense skill next turn.
- not partial commit — every firing prong applies or none.
- not retune thresholds (20k-token advisory, > 50 closed-§T archive trigger) in this skill body — canonical values live in the token-budget-condense invariant row (SPEC.md) w/ mechanical mirrors in `check-mechanical.py` constants; retune via invoking the spec skill AMEND + sync the script constant same commit.